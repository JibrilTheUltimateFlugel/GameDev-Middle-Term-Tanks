...
public class GameManager : MonoBehaviour
{
    ...
    private GameObject m_Hitscreen; // game object reference of the Red Screen Tinge
    ...
    private void Start()
    {
        ...
        m_Hitscreen = GameObject.Find("RedScreen");
    ...
    // To be raised when OnPlayerDamage is called
    public void Damage()
    {
        StartCoroutine(Fade());
    }

    // Coroutine for Fading UI
    IEnumerator Fade()
    {
        for (float ft = 1f; ft >= 0; ft -= 0.02f)
        {
            Color c = m_Hitscreen.GetComponent<Image>().color; //get the color of the Image component of the UI
            c.a = ft; //set the alpha to ft
            m_Hitscreen.GetComponent<Image>().color = c; //set color to the new alpha
            yield return null;
        }
    }
}